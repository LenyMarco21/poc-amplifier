...
#include "tad5212_default_config.h"

/*** Defines ***********************************************************************/

/* I2C operation timeout in milliseconds */
#define TAD5212_I2C_WRITE_TIMEOUT_MS    -1

/* I2C operation timeout in milliseconds */
#define TAD5212_I2C_READ_TIMEOUT_MS     100
...
/** \brief Read a register from the TAD5212 device.
 *
...
inline static esp_err_t select_page(uint8_t page)
{
    return write_register(REG_PAGE_CFG, page);
}
...
esp_err_t tad5212_init(i2c_port_t i2c_port, int gpio_sda, int gpio_scl, int i2c_freq, tad5212_i2c_addr_t i2c_addr)
{
    ...
    status = write_register(REG_SW_RESET, DEFAULT_SW_RESET.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_DEV_MISC_CFG, DEFAULT_DEV_MISC_CFG_P0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_DAC_CFG_A0, DEFAULT_DAC_CFG_A0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_MISC_CFG0, DEFAULT_MISC_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_INTF_CFG1, DEFAULT_INTF_CFG1.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_ASI_CFG1, DEFAULT_ASI_CFG1.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_PASI_CFG0, DEFAULT_PASI_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_PASI_RX_CH1_CFG, DEFAULT_PASI_RX_CH1_CFG.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_PASI_RX_CH2_CFG, DEFAULT_PASI_RX_CH2_CFG.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT1X_CFG0, DEFAULT_OUT1X_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT1X_CFG1, DEFAULT_OUT1X_CFG1.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT1X_CFG2, DEFAULT_OUT1X_CFG2.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_DAC_CH1A_CFG0, DEFAULT_DAC_CH1A_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT2X_CFG0, DEFAULT_OUT2X_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT2X_CFG1, DEFAULT_OUT2X_CFG1.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_OUT2X_CFG2, DEFAULT_OUT2X_CFG2.data);
    if (status != ESP_OK)
    {
    ...
    status = write_register(REG_DAC_CH2A_CFG0, DEFAULT_DAC_CH2A_CFG0.data);
    if (status != ESP_OK)
    {
    ...
    /* Enable channels */
    status = write_register(REG_CH_EN, DEFAULT_CH_EN.data);
    if (status != ESP_OK)
    {
    ...
    /* Power up the device */
    status = write_register(REG_PWR_CFG, DEFAULT_PWR_CFG.data);
    if (status != ESP_OK)
    {
...
esp_err_t tad5212_set_volume(tad5212_channel_t channel, uint8_t volume)
{
    ...
    uint8_t dvol_value = TAD5212_DAC_MIN_VOLUME + (uint8_t)(TAD5212_DAC_VOLUME_STEP * volume);
    ...
    if (channel == TAD5212_CHANNEL_LEFT || channel == TAD5212_CHANNEL_BOTH) 
    {
        ...
        status = write_register(REG_DAC_CH1A_CFG0, dac_ch1a_cfg0.data);
    ...
    if (channel == TAD5212_CHANNEL_RIGHT || channel == TAD5212_CHANNEL_BOTH) 
    {
        ...
        status = write_register(REG_DAC_CH2A_CFG0, dac_ch2a_cfg0.data);
...
esp_err_t tad5212_swap_channels(void)
{
    ...
    uint8_t reg_value = 0;
    esp_err_t status = read_register(REG_DYN_PUPD_CFG, &reg_value);
    ...
    status = write_register(REG_DYN_PUPD_CFG, reg_value);
...
esp_err_t tad5212_dac_status(void)
{
    ...
    esp_err_t status = read_register(REG_DEV_STS0, &reg_value_1);
    ...
    status = read_register(REG_DEV_STS1, &reg_value_2);
    ...
    status = read_register(REG_CLK_ERR_STS0, &reg_value_3);
    ...
    status = read_register(REG_CLK_ERR_STS1, &reg_value_4);
    ...
    status = read_register(REG_CLK_DET_STS0, &reg_value_5);
    ...
    status = read_register(REG_CLK_DET_STS2, &reg_value_6);
    ...
    status = read_register(REG_CLK_DET_STS3, &reg_value_7);
